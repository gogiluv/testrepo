    ...
    for line in lines:
        ...
        if labels_flag == True:
            if "[" in line:
                ...
                problem_num = line.split('.')[0]
                label = line.split('.')[1].split('/')[0]
                # label = list(set(line.split('.')[1].split('/')[0].split(',').upper().split(',')))
                assigned_score = line.split('/')[1][:-1]
                labels[category_name][problem_num] = [label.upper(), int(assigned_score)]
...
class SubmissionAPI(APIView):
    ...
    def duplicate_chk(self, user_id, problem_id):
        submission = Submission.objects.get(problem_id=problem_id, user_id=user_id)
...
class SubmissionFromUserAPI(APIView):
    def get(self, request):
        ...
        data = self.paginate_data(request, submissions)
        data["results"] = SubmissionByUserSerializer(data["results"], many=True, user=request.user).data
